여기에서 만든 사용자 지정 컨테이너 이미지를 공용 컨테이너 레지스트리에 이러한 이미지를 푸시 및 이러한 이미지에서 컨테이너를 실행 합니다.

컨테이너 이미지를 직접 만들 수 있습니다 하거나 Dockerfile을 사용 하 여 프로세스를 자동화 합니다. 선호 되는 방법에는 Dockerfile을 사용 하는 있지만이 단위에서는 두 가지 방법을 모두 보여 줍니다. 수동 프로세스를 캡처하므로 자동화에 대 한 Dockerfile을 사용 하는 경우 문제가 무엇 인지 파악 하는 데 도움이 됩니다.
...
Python을 실행 하는 컨테이너의 인스턴스를 시작 합니다.이 첫 번째 예에서는 ' Hello World' 응용 프로그램을 및 다음 컨테이너를 새 이미지를 캡처합니다.

먼저 NGINX 이미지에서 컨테이너를 실행합니다. 이 명령은 이전 섹션에서 실행한 명령과 약간 다르게 보입니다. 실행 중인 컨테이너와 터미널 세션을 설정하려는 것이기 때문에 `-t` 및 `-i` 인수를 입력합니다. 함께 이러한 인수는 실행 중 상태로 남아 있는 터미널 의사를 할당 하는 Docker를 지시 합니다. 즉, `-t` 및 `-i` 인수는 실행 중인 컨테이너와 대화형 세션을 만듭니다.
...
명령을 실행하면 터미널 세션이 컨테이너 의사 터미널로 전환됩니다. 다음과 같이 변경 해야 하는 터미널 프롬프트에서이 확인할 수 있습니다.

```output
root@d8ccada9c61e:/#
```

지금은 컨테이너 내부에서 작업합니다. 목록, 만들고, 및 파일 삭제, 소프트웨어를 설치 하 구성을 변경 합니다. 이 간단한 예제에서는 Python 기반 ' Hello World' 스크립트 생성 됩니다. 이 작업은 다음 명령을 사용하여 수행할 수 있습니다.
...
만족 하는 스크립트가 예상 대로 작동 하는 경우 입력 하 여 컨테이너를 종료 `exit`:
...
아무 것도 실행되고 있지 않습니다. 실행 중인 컨테이너에서 `exit`를 입력하면 Bash 프로세스가 완료되고 컨테이너가 중지됩니다. 이 동작은 예상 합니다.
...
이름이 *python-demo*인 컨테이너의 상태는 *끝냄*입니다. 이 컨테이너는 방금 전에 종료 하는 컨테이너의 중지 된 인스턴스입니다.
...
컨테이너 시작 되며 ' Hello World' 메시지를 출력 합니다. 그 후 Python 프로세스가 완료되고 컨테이너가 중지됩니다.
...
마지막 섹션에서 만든 컨테이너 이미지를 수동으로. 이 메서드는 일회성 또는 실험적 이미지를 만들기 위한 작동 하는 동안 프로덕션 환경에서 유지 가능한 아닙니다. *Dockerfile* 및 `docker build` 명령을 사용하여 컨테이너 이미지 만들기를 자동화할 수 있습니다. 합니다 *docker 빌드* 명령은 컨테이너를 시작, 실행에서 지침을 찾을 수 합니다 *Dockerfile*를 하 고 다음 결과를 새 컨테이너 이미지를 캡처합니다.
...
컨테이너 이미지를 Docker 허브에 업로드 하는 동안 다음과 유사한 출력이 표시 됩니다.
...
컨테이너 이미지를 Docker 허브에 저장 됩니다 하 고 사용 하 여 모든 인터넷에 연결 된 컴퓨터에서 액세스할 수 있습니다 `docker pull` 또는 `docker run`합니다.

두 개의 컨테이너 이미지를 만들었습니다. 첫 번째 이미지는 수동으로 만들었고 두 번째 이미지는 Dockerfile을 사용하여 자동화했습니다.