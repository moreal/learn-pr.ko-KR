작동하는 컨테이너 개발 환경이 생겼으니, 기본적인 컨테이너 작업 몇 가지를 간략하게 살펴보겠습니다. 이번 섹션에는 전체 Docker 기능 목록이 포함되지 않습니다(비슷하지도 않음). 이번 섹션에서는 컨테이너를 실행, 나열 및 삭제하기 위한 준비를 할 것입니다. 이 모듈의 나머지 부분에서는 컨테이너 작업을 추가로 공개하겠습니다.
