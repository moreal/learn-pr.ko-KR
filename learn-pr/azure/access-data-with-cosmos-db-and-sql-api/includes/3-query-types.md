...
FROM 절은 쿼리가 작동하는 데이터 소스를 지정합니다. 컬렉션 전체를 쿼리의 소스로 지정하거나 컬렉션의 하위 집합을 대신 지정할 수 있습니다. 쿼리의 뒷부분에서 소스를 필터링/프로젝션하지 않을 경우 FROM 절은 선택 사항입니다.

`SELECT * FROM Products`와 같은 쿼리는 전체 Products 컬렉션이 쿼리를 열거할 소스임을 나타냅니다.
...
별칭으로 지정한 후에는 원본 소스를 바인딩할 수 없습니다. 예를 들어 `SELECT Products.id FROM Products p`는 "Products" 식별자를 더 이상 확인할 수 없으므로 구문이 잘못되었습니다.

참조해야 하는 모든 속성은 정규화되어야 합니다. 엄격한 스키마 준수가 없을 경우 모호한 바인딩을 방지하기 위해 적용됩니다. 따라서 `SELECT id FROM Products p`는 `id` 속성이 바인딩되지 않았으므로 구문이 잘못되었습니다.

### <a name="subdocuments-in-a-from-clause"></a>FROM 절의 하위 문서
소스를 더 작은 하위 집합으로 줄일 수도 있습니다. 예를 들어 각 문서에서 하위 트리만을 열거하려는 경우 다음 예제처럼 하위 루트가 소스가 될 수 있습니다.
...
위 예제에서는 배열을 소스로 사용했지만 다음 예제의 경우처럼 개체를 소스로 사용할 수도 있습니다. 소스에 있는 유효한 모든 JSON 값(undefined 제외)이 쿼리 결과에 포함되기 위해 고려됩니다. 일부 제품에 `shipping.weight` 값이 없는 경우 쿼리 결과에서 제외됩니다.
...
JOIN 절을 사용하면 문서 및 하위 루트와 내부 조인을 수행 할 수 있습니다. 예를 들어 제품 데이터베이스에서 운송 데이터로 문서를 조인할 수 있습니다.  
