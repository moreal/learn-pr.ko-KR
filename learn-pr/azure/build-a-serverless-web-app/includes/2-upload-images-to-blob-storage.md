빌드하는 응용 프로그램은 사진 갤러리입니다. API를 호출하는 클라이언트 쪽 JavaScript를 사용하여 이미지를 업로드하고 표시합니다. 이 단위에 이미지 업로드에 대 한 시간이 제한 된 URL을 생성 하는 서버 리스 함수를 사용 하 여 API 만들게 됩니다. 웹 응용 프로그램을 사용 하 여 Blob storage에 이미지를 업로드 하려면이 URL을 사용 합니다 [Blob storage REST API](https://docs.microsoft.com/rest/api/storageservices/blob-service-rest-api)합니다.
...
1.  라는 저장소 계정에 새 컨테이너를 만듭니다 **이미지** 저장소 계정의 모든 blob에 대 한 공용 액세스를 사용 하 여 합니다.
...
함수 앱은 하나 이상의 서버 리스 functions에 대 한 컨테이너입니다.

- 고유한 이름의 새 함수 앱 만들기를 **첫 번째 서버 리스 앱** 앞에서 만든 리소스 그룹입니다. 함수 앱에는 저장소 계정이 필요 합니다. 이 장치에서 마지막 단위에서 만든 기존 저장소 계정을 사용 합니다.
...
1. 함수 앱을 만든 후에 검색 합니다 [Azure portal](https://portal.azure.com/?azure-portal=true) 를 사용 하는 **검색** 상자. 앱을 클릭하여 엽니다.

    ![함수 앱 열기](../media/2-search-function-app.png)

1. 함수 앱 창의 왼쪽 탐색에서 가리킨 **함수** 새 서버 리스 함수를 만들려면 더하기 기호 (+)를 클릭 합니다.

    ![새 함수 만들기](../media/2-new-function.png)

1. **사용자 지정 함수**를 클릭하여 함수 템플릿 목록을 확인합니다.

1. 찾을 합니다 **HttpTrigger** 템플릿 C# 또는 JavaScript를 클릭 합니다.
...
1. 다음 값을 사용하여 Blob 업로드 URL을 생성하는 함수를 만듭니다.
    ...
    | **권한 부여 수준** | 익명 | 함수를에 공개적으로 액세스할 수 있습니다. |
...
::: zone pivot="csharp"
1. (C#) 함수의 소스 코드에 표시 되 면 콘텐츠 모두 바꾸기 합니다 **run.csx** 의 콘텐츠를 사용 하 여 파일을 [ **csharp/GetUploadUrl/run.csx** ](https://raw.githubusercontent.com/Azure-Samples/functions-first-serverless-web-application/master/csharp/GetUploadUrl/run.csx) 파일.
...
함수는 Blob storage에 파일을 업로드 하는 데 사용 되는 공유 액세스 서명 (SAS) URL을 생성 합니다. SAS URL는 짧은 시간 동안 유효 하며만 단일 파일에서 업로드할 수 있습니다. 자세한 정보에 대 한 Blob 저장소 설명서를 참조 하세요 [공유 액세스 서명을 사용 하는 방법을](https://docs.microsoft.com/azure/storage/common/storage-dotnet-shared-access-signature-part-1)합니다.


## <a name="add-an-environment-variable-for-the-storage-connection-string"></a>저장소 연결 문자열에 대한 환경 변수 추가

SAS URL을 생성할 수 있도록 직접 만든 함수에는 저장소 계정에 대한 연결 문자열이 필요합니다. 함수의 본문에서 연결 문자열을 하드 코드하는 대신 응용 프로그램 설정으로 저장할 수 있습니다. 응용 프로그램 설정을 함수 앱에서 모든 함수에서 환경 변수로 액세스할 수 있습니다.
...
함수 프런트 엔드는 Blob storage에서 호스팅되므로 함수 앱과 다른 도메인 이름을 있습니다. 성공적으로 만든 함수를 호출 하려면 클라이언트 쪽 JavaScript에 대 한 함수 앱 크로스-원본 자원 공유 (CORS)에 대해 구성 해야 합니다.

1. 함수 앱 창의 왼쪽된 탐색 영역에서 함수 앱의 이름을 클릭 합니다.
...
1. 이전 단위에서 만든 웹 사이트의 URL에 대 한 허용 origin을 추가 하 고 후행 슬래시 (/)를 생략 합니다. 예: `https://firstserverlessweb.z4.web.core.windows.net`.
...
1. Azure portal에서 함수 앱으로 이동 합니다. **개요** 탭을 선택합니다. **다시 시작**을 클릭하여 CORS에 대한 변경 내용이 적용되는지 확인합니다.

## <a name="configure-cors-in-the-storage-account"></a>저장소 계정에서 CORS 구성

함수 앱 파일을 업로드 하려면 Blob 저장소로 클라이언트 쪽 JavaScript 호출도 하므로 CORS에 대 한 저장소 계정을 구성 해야 합니다.
...
웹앱은 **settings.js**라는 파일에서 설정을 검색합니다. 다음 단계에서는 Cloud Shell을 사용하여 파일을 만듭니다. 설정한 `window.apiBaseUrl` 함수 앱의 url 및 `window.blobBaseUrl` Azure Blob storage 끝점의 url입니다.
...
이 단원에서는 Azure Functions 앱을 만들고 서버리스 함수를 사용하여 웹 응용 프로그램이 Blob Storage에 이미지를 업로드하도록 하는 방법을 알아보았습니다. 그런 다음 blob 트리거는 서버 리스 함수를 사용 하 여 업로드 된 이미지에 대 한 미리 보기를 만드는 방법을 배웁니다.