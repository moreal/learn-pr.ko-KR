여기에서는 Azure portal을 사용 하 여 Azure App Service에서 웹 앱을 만드는 방법에 알아봅니다.

## <a name="why-use-the-azure-portal"></a>Azure Portal을 사용하는 이유는 무엇인가요?

웹 응용 프로그램을 호스트 하는 첫 번째 단계는 Azure 구독 내에서 웹 앱 (App Service 앱)을 만드는 것입니다.

여러 가지 방법으로 웹 앱을 만들 수 있습니다. Azure Portal, Azure CLI, 스크립트 또는 IDE를 사용할 수 있습니다.
...
Azure App Service는 웹 앱, REST Api 및 모바일 백 엔드를 호스트 하기 위한 최적화 된 Azure 환경 내에서 완전히 관리 되는 컴퓨팅 플랫폼입니다.
...
## <a name="how-to-create-a-web-app"></a>웹 앱을 만드는 방법

사용자 고유의 앱을 호스트 하는 데 시간이 되 면 Azure portal을 방문 하 고 만들 있습니다를 **웹 앱**합니다. 만들어를 **웹 앱** 호스팅 ASP.NET Core, Node.js, PHP 등 든 Azure에서 지원 되는 모든 웹 기반 응용 프로그램을 호스트 하는 데 사용할 수 있는 App Service에서 리소스 집합을 만드는 실제로 Azure portal에서 합니다. 아래 그림은 앱에서 사용하는 프레임워크/언어를 구성하기가 얼마나 쉬운지를 보여 줍니다.

![웹앱 설정](../media/2-web-app-settings.png)

Azure portal에는 웹 앱을 만드는 템플릿을 제공 합니다. 이 템플릿에는 다음 필드가 필요합니다.
...
Azure portal을 사용 하 여 쉽게 추가할 수 있습니다 **배포 슬롯** App Service 웹 앱에 있습니다. 예를 들어 Azure에서 테스트하기 위해 코드를 푸시할 수 있는 **스테이징** 배포 슬롯을 만들 수 있습니다. 코드에 만족할 경우 스테이징 배포 슬롯을 프로덕션 슬롯과 쉽게 **교환**할 수 있습니다. Azure Portal에서 마우스를 몇 번 클릭하면 이 작업을 모두 수행할 수 있습니다.

![배포 슬롯](../media/2-deployment-slots.png)

### <a name="continuous-integrationdeployment-support"></a>연속 통합/지속적인 배포 지원

Azure portal 개발 컴퓨터에 기본 제공 연속 통합 및 Visual Studio Team Services, GitHub, Bitbucket, Dropbox, OneDrive 또는 로컬 Git 리포지토리를 사용 하 여 배포를 제공합니다. 위의 소스 중 하나를 사용 하 여 웹 앱을 연결 하 고 App Service 웹 앱에 코드 자동 동기화 하 고 코드에 대 한 이후 모든 변경 사항에 대 한 나머지에 수행 됩니다. 또한 Visual Studio Team Services를 통해 고유한 빌드 및 릴리스 프로세스를 정의하여 원본 코드를 컴파일하고, 테스트를 실행하고, 릴리스를 빌드하고, 마지막으로 코드를 커밋할 때마다 릴리스를 웹앱에 푸시할 수 있습니다. 모든 작업은 개입 없이 암시적으로 수행됩니다.
...
### <a name="built-in-auto-scale-support-automatic-scale-out-based-on-real-world-load"></a>기본 제공 자동 크기 조정 지원 (자동 스케일 아웃 실제 부하에 따라)
...
웹 앱, 모바일 앱 및 Azure Functions, App Service 계획에서 모두 실행된 뿐만 아니라 Azure App Service에서 호스팅되는 API 앱입니다. 개수에 제한 없이 응용 프로그램을 App Service 계획에 배포할 수 있지만, 사용하는 개수는 배포된 응용 프로그램 유형 및 CPU 사용률에 필요한 리소스에 따라 크기 달라집니다.
