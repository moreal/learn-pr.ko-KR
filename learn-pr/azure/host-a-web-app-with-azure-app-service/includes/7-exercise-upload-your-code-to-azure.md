...
1. Azure Portal은 아래와 같이 **배포 자격 증명** 페이지로 이동합니다.
    ...
    선택한 **사용자 이름**과 **암호**를 입력한 다음, 암호를 다시 한번 확인합니다.

    > [!NOTE]
    > 사용자 이름과 암호를 잊지 않도록 하세요. 사용자 이름과 암호는 나중에 Azure에 코드 업로드 및 배포를 시작할 때 필요합니다.
...
1. 이제 왼쪽 탐색 영역에서 **개요** 섹션으로 이동합니다.

    여기서 주목해야 하는 중요한 정보는 **Git Clone URI**입니다. 이 URI는 로컬 응용 프로그램 코드 리포지토리의 **원격** 항목으로 사용할 로컬 Git 리포지토리 URL입니다.

이제 스테이징 배포 슬롯에 코드 업로드를 시작하겠습니다.

## <a name="set-up-git-on-cloud-shell"></a>Cloud Shell에서 git 설정

Git가 이미 Azure Cloud Shell을 설치했지만 클라우드 셸 계정에 대한 사용자 이름과 이메일을 설정하려고 합니다.

1. 오른쪽의 Cloud Shell에 다음 명령을 입력하고 `[your name]` 및 `[your email]` 자리 표시자를 사용자의 이름과 이메일(중괄호 제외)로 바꿉니다.
...
1. 위에서 확인한 **Git 복제 URL**을 복사합니다.

1. 복사한 후 **터미널** 창으로 돌아가서 URL을 사용하여 다음 Git 명령을 실행합니다.
...
## <a name="verify-the-code-is-uploaded-to-azure"></a>코드가 Azure에 업로드되었는지 확인
...
1. 작업이 종료되면 웹앱 URL을 방문하세요. [https://bestbike.azurewebsites.net/](https://bestbike.azurewebsites.net/) 포털의 앱 서비스에 대한 개요 페이지에서 찾을 수 있습니다.
