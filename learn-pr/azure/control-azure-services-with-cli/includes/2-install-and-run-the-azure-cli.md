회사에서 Azure 리소스를 관리하기 위한 명령줄 솔루션으로 Azure CLI를 선택했다고 가정합니다. 관리자 및 개발자는 브라우저를 사용하는 것보다 명령 및 스크립트를 로컬로 실행하는 것을 선호합니다. 팀은 Linux, macOS 및 Windows를 실행하는 머신을 사용합니다. Azure CLI가 모든 장치에서 작동해야 합니다.
