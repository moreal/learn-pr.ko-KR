...
## <a name="how-the-cloud-changes-your-expenses"></a>클라우드 비용이 달라지는 원리
...
클라우드로 전환하면 종량제 비용 모델이 적용됩니다. 더 이상 투자 비용을 자산에 묶어둘 필요가 없으며, 리소스 요구 사항이 바뀌면 그에 맞게 리소스를 추가, 이동 또는 제거할 수 있습니다. 워크로드는 서비스마다 그리고 서비스 내에서 달라지고, 수요는 예측 불가능하며, 성장 패턴은 시간에 따라 달라집니다. 클라우드에서 사용한 만큼만 지불하므로 리소스가 변하면 비용 구조도 그에 따라 변화합니다.

다음 다이어그램은 클라우드 인프라를 사용하여 쉽게 수용할 수 있는 일반적인 리소스 사용 패턴을 보여줍니다. 장기간 사용하지 않는 리소스를 종료하면 비용이 전혀 발생하지 않습니다. 리소스를 동적으로 추가 또는 제거하여 예측 가능한 수요와 예측 불가능한 수요에 대응할 수 있습니다. 다음 조달 주기를 기다리지 않고, 서비스의 성장에 따라 리소스를 늘릴 수 있습니다.

![수요 곡선](../media-draft/cloudcomputingpatterns.png)

효율적인 아키텍처에서는 프로비전된 리소스가 수요와 일치합니다. 대부분의 시간 동안 가상 머신의 사용률이 10% 미만이면 계산의 측면에서 그리고 비용의 측면에서 리소스를 낭비하고 있는 것입니다. 반대로, 사용률이 90%인 가상 머신은 가용 리소스를 대부분 사용하고 있으므로 비용을 효율적으로 사용하는 것입니다. 시스템 사용률이 100%이면 성능 문제가 발생할 위험이 있습니다. 효율성 극대화가 시스템 성능에 악영향을 주지 않도록 주의해야 합니다. 수요는 일정하지 않으므로, 가능하다면 수요에 맞게 리소스를 조정하여 효율을 높이는 것이 중요합니다.
...
구독 및 리소스 그룹을 사용하여 리소스를 구성하면 간단하게 리소스를 논리적으로 그룹화하여 청구 데이터를 진행할 때 활용할 수 있습니다. 태그는 리소스 관계가 구독과 리소스 그룹의 경계에 걸쳐 있을 때 중요한 역할을 합니다. 태그는 모든 리소스에 추가할 수 있는 키/값 쌍으로, 청구 데이터에 공개되며 부서 또는 비용 센터를 리소스와 연결할 수 있게 해줍니다. 태그는 비용에 대한 보고 능력을 향상할 뿐 아니라 조직 내 각 부서에 비용 책임을 명확하게 합니다.

![리소스 태그 지정](../media-draft/tagging.png)

리소스에 일부 조직을 추가하면 많은 도움이 될 수 있으며 비용이 어디에 투입되는지 이해하는 능력에 매우 큰 도움이 될 수 있습니다. 이번에는 비용을 최적화하는 몇 가지 방법을 살펴보겠습니다.
...
가상 머신을 적절한 크기로 조정하는 것은 가상 머신 크기를 VM에서 요구하는 리소스 수요와 일치시키는 프로세스입니다. VM이 25% 유휴 상태인 경우 VM 크기를 줄이면 즉시 비용이 절감됩니다. 가상 머신 비용은 인스턴스 패밀리 내에서 선형적이며, 크기가 한 단계 커질 때마다 비용이 두 배로 늘어납니다. 반대로, VM의 인스턴스 크기를 하나 줄이면 비용이 절반으로 감소합니다.

![가상 머신 크기 조정](../media-draft/vm-resize.png)

Azure Advisor는 사용률이 낮은 가상 머신을 식별합니다. Advisor는 14일 동안 가상 머신 사용량을 모니터링하고 사용률이 낮은 가상 머신을 식별합니다. 4일 이상 CPU 사용률이 5% 이하이고 네트워크 사용량이 7MB 이하인 가상 머신은 사용률이 낮은 가상 머신으로 간주됩니다.
...
풀에 더 많은 데이터베이스를 추가할수록 더 많은 비용이 절감됩니다.

![탄력적 풀](../media-draft/sqldb-elastic-pools.png)
