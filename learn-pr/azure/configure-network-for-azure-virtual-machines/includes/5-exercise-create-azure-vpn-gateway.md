...
1. 인증을 위해 루트 인증서의 공개 키를 업로드합니다.

1. 루트 인증서에서 클라이언트 인증서를 생성한 다음, 인증을 위해 가상 네트워크에 연결할 각 클라이언트 컴퓨터에 클라이언트 인증서를 설치합니다.

1. 클라이언트를 가상 네트워크에 연결하는 데 필요한 정보가 포함된 VPN 클라이언트 구성 파일을 만듭니다.

## <a name="before-you-begin"></a>시작하기 전에
<!---TODO: These should be prerequisites in the first unit and on the index.yml--->

이 모듈을 완료하려면 다음 항목이 필요합니다.
...
1. PowerShell 창에서 다음 명령을 입력하고 Enter 키를 누릅니다.
...
1. 다음 변수를 입력하고, 각 변수 다음에 Enter 키를 누릅니다.
...
1. 서브넷 값 및 정적 DNS 서버를 사용하여 가상 네트워크를 만듭니다.
    ...
    > [!IMPORTANT]
    > 경고 메시지를 무시한 다음, 명령이 종료될 때까지 기다립니다.
...
## <a name="create-the-vpn-gateway"></a>VPN 게이트웨이 만들기
...
> [!NOTE]
> 연습의 이 부분은 GatewaySku의 값에 따라 완료하는 데 최대 45분이 걸릴 수 있습니다.
...
1. **아니요, 개인 키를 내보내지 않습니다.** 를 선택했는지 확인한 후, **다음**을 클릭합니다.

1. **내보내기 파일 형식** 페이지에서 **Base 64로 인코딩된 X.509(.CER)** 를 선택했는지 확인한 후, **다음**을 클릭합니다.

1. **내보낼 파일** 페이지의 **파일 이름**에 **C:\cert\P2SRootCert.cer**을 입력한 후, 다음을 클릭합니다.
...
1. 이제 Azure에 인증서를 업로드합니다. 그러면 Azure에서 신뢰할 수 있는 루트 인증서로 인식됩니다.
...
1. 이 명령의 출력에 반환된 URL을 복사하여 브라우저에 붙여넣습니다. 브라우저에서 .ZIP 파일 다운로드가 시작됩니다. 압축을 풀고 적절한 위치에 배치합니다.

1. 추출된 폴더에서 WindowsAmd64 폴더(64비트 Windows 컴퓨터의 경우) 또는 WindowsZX86 폴더(32비트 Windows 컴퓨터의 경우) 중 하나로 이동합니다.

1. 아키텍처에 따라 VpnClientSetupxxxxx.exe 파일을 두 번 클릭합니다.

1. **Windows의 PC 보호** 화면에서 **자세한 정보**를 클릭한 다음, **실행**을 클릭합니다.
...
1. Windows 키를 누르고, **설정**을 입력하고, Enter 키를 누릅니다.
...
1. Windows 키를 누르고, **cmd**를 입력하고, Enter 키를 누릅니다. **명령 프롬프트** 창이 나타납니다.

1. `IPCONFIG /ALL`를 입력하고 Enter 키를 누릅니다.

1. PPP 어댑터 VNetData의 IP 주소를 복사하거나 적어둡니다.

1. IP 주소가 **172.16.201.0/24 VPNClientAddressPool 범위**에 있는지 확인합니다.

1. Azure VPN Gateway에 성공적으로 연결했습니다.
