...
관리 디스크는 권장되는 최신 디스크 저장소 모델입니다. 저장소 계정을 관리하는 부담이 Azure에서 해결되기 때문에 복잡한 업무가 원활하게 해결됩니다. 사용자가 디스크 유형(프리미엄 또는 표준)과 디스크 크기를 지정하면, Azure에서 사용되는 디스크와 저장소가 _모두_ 만들어지고 관리됩니다. 저장소 계정 제한에 대해 걱정할 필요가 없으므로 규모 확장이 쉬워집니다. 또한 다음과 같은 몇 가지 다른 이점도 제공합니다.
