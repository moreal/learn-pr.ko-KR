로컬 Ubuntu Linux 서버에서 실행 중인 기존 웹 사이트가 있습니다. 최신 Ubuntu 이미지를 사용하여 Azure VM(가상 머신)을 만든 후 사이트를 클라우드로 마이그레이션하는 것이 목표입니다. 이 단원에서는 Azure에서 가상 머신을 만들 때 평가해야 하는 옵션에 대해 알아봅니다.

## <a name="introduction-to-azure-virtual-machines"></a>Azure Virtual Machines 소개

Azure Virtual Machines는 요청 시 확장 가능한 클라우드 컴퓨팅 리소스입니다. 프로세서, 메모리, 저장소 및 네트워킹 리소스가 여기에 포함됩니다. 마음대로 가상 머신을 시작하고 중지할 수 있으며, Azure Portal 또는 Azure CLI를 통해 관리할 수 있습니다. 원격 SSH(Secure Shell)를 사용하여 실행 중인 VM에 직접 연결하고 로컬 컴퓨터에서 작업하듯이 명령을 실행할 수도 있습니다.
...
VM은 Azure Portal, 스크립트(Azure CLI 또는 Azure PowerShell 사용) 또는 Azure Resource Manager 템플릿 등의 여러 가지 방법으로 Azure에서 정의하고 배포할 수 있습니다. 모든 경우에 곧 설명할 몇 가지 정보를 제공해야 합니다.
...
Azure에서 Linux VM을 만드는 경우 VM을 호스팅할 리소스도 만듭니다. 이러한 리소스는 함께 작동하여 컴퓨터를 가상화하고 Linux 운영 체제를 실행합니다. 다음 리소스가 반드시 존재해야 하며(또한 VM을 만드는 중에 선택되어야 함), 아니면 VM과 함께 만들어집니다.
...
> [!TIP]
> 사용자 지정 디스크 이미지를 만들고 업로드할 수도 있습니다.
...
> [!WARNING]
> VM을 만드는 데 영향을 줄 수 있는 각 구독에 대한 할당량 제한이 있습니다. 이러한 할당량 제한에 도달하는 경우 [온라인 고객 지원 요청을 열어](https://docs.microsoft.com/azure/azure-supportability/resource-manager-core-quotas-request) 제한을 늘릴 수 있습니다.

VM 크기는 기본 테스트를 위한 B 시리즈부터 시작하여 대규모 컴퓨팅 작업을 위한 H 시리즈까지의 범주로 그룹화됩니다. 수행하려는 워크로드에 따라 VM의 크기를 선택해야 합니다. VM을 만든 후에 크기를 변경할 수는 있지만 VM을 먼저 중지해야 합니다. 따라서 가능하면 처음부터 적절하게 크기를 정하는 것이 가장 좋습니다.
...
| **HPC(고성능 컴퓨팅)**: 워크로드에 선택적인 높은 처리량 네트워크 인터페이스를 갖춘 가장 빠르고 강력한 CPU 가상 머신이 필요한 경우 | H |
