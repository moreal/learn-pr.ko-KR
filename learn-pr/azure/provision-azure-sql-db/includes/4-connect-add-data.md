...
Microsoft는 Azure SQL Database에 대한 인프라, 소프트웨어 업데이트 및 패치를 유지 관리하고 있습니다. 또한 다른 SQL Server 설치와 마찬가지로 Azure SQL Database를 관리할 수 있습니다. 예를 들어, Visual Studio, SQL Server Management Studio 또는 기타 도구를 사용하여 Azure SQL Database를 관리할 수 있습니다.

데이터베이스에 액세스한 후 앱에 연결할 때는 사용자가 원하는 방법을 사용할 수 있습니다. 그러나 여기서는 데이터베이스로 여러 작업을 수행해보기 위해 포털에서 직접 연결하고, 테이블을 만들고, 몇 가지 기본 CRUD 작업을 실행합니다. 다음 내용을 배웁니다.
...
내부적으로, Cloud Shell은 Linux에서 실행됩니다. 그러나 Linux 또는 Windows 중에서 어떤 환경을 선호하는지에 따라 Bash 및 PowerShell 중에서 선택할 수 있습니다.

Cloud Shell은 어디에서나 액세스할 수 있습니다. 포털 외에도 [shell.azure.com](https://shell.azure.com/), Azure 모바일 앱 또는 Visual Studio Code에서 Cloud Shell에 액세스할 수 있습니다.
...
* `sqlcmd`를 사용하여 SQL Server에서 문을 실행할 수 있습니다. `sqlcmd`를 사용하여 Azure SQL Database와의 대화형 세션을 만듭니다.

## <a name="get-information-about-your-azure-sql-database"></a>Azure SQL Database에 대한 정보 가져오기

데이터베이스에 연결하기 전에 해당 데이터베이스가 존재하는지와 온라인 상태인지 확인하는 것이 좋습니다.

여기서는 Cloud Shell을 불러온 후 `az` 유틸리티를 사용하여 데이터베이스를 나열하고, 최대 크기 및 상태를 비롯한 **Logistics** 데이터베이스 관련 정보를 표시합니다.

1. 포털 맨 위에서 **Cloud Shell**을 클릭합니다. 
...
CRUD는 **만들기**, **읽기**, **업데이트** 및 **삭제**를 의미합니다. 이것은 테이블 데이터에 대해 수행하는 작업을 나타내며, 앱에 필요한 네 가지 기본 작업에 해당합니다. 이제 각 작업을 수행할 수 있는지 확인해보겠습니다.
...
1. 이 `az sql db show-connection-string` 명령을 실행하여 연결 문자열을 `sqlcmd`에서 사용할 수 있는 형식으로 **Logistics** 데이터베이스에 가져옵니다.
    ...
    ```
1. 이전 단계의 `sqlcmd` 문을 실행하여 대화형 세션을 만듭니다.
    따옴표를 제거하고 `<username>` 및 `<password>`를 사용자가 데이터베이스를 만들 때 지정한 사용자 이름과 암호로 바꿉니다. 예를 들면 다음과 같습니다.
    ```console
...
1. 이 `INSERT` T-SQL 문을 실행하여 테이블에 샘플 행을 추가합니다. 이것은 **만들기** 작업입니다.
    ...
    GO
    ```
    작업이 성공했음을 나타내는 다음 내용이 표시됩니다.
    ```console
    (1 rows affected)
    ```
1. 이 `SELECT` T-SQL 문을 실행하여 테이블의 모든 행에 있는 `DriverID` 열을 나열합니다. 이것은 **읽기** 작업입니다.
    ```sql
    SELECT DriverID FROM Drivers;
    GO
    ```
    하나의 결과, 즉 이전 단계에서 만든 행에 대한 `DriverID`가 표시됩니다.
    ```console
    DriverID
    -----------
            123
...
Cloud Shell을 사용하면 Azure 리소스에 쉽게 액세스하고 관련 작업을 수행할 수 있습니다. Cloud Shell은 브라우저 기반이므로 Windows, macOS 또는 Linux를 비롯하여 웹 브라우저가 있는 모든 시스템에서 액세스할 수 있습니다.

Azure CLI 명령을 실행하여 SQL Azure Database에 대한 정보를 가져오는 방법에 대한 몇 가지 실습 경험을 얻었습니다. 추가로 T-SQL 기술도 연습해보았습니다.
