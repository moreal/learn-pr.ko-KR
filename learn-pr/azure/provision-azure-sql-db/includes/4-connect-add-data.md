...
Microsoft는 Azure SQL Database에 대한 인프라, 소프트웨어 업데이트 및 패치를 유지 관리합니다. 또한 기타 모든 SQL Server 설치와 마찬가지로 Azure SQL Database를 관리할 수 있습니다. 예를 들어, Azure SQL database를 관리 하려면 Visual Studio, SQL Server Management Studio, SQL Server Operations Studio 또는 다른 도구를 사용할 수 있습니다.
...
Cloud Shell은 어디에서나 액세스할 수 있습니다. 포털 외에도 [shell.azure.com](https://shell.azure.com/), Azure 모바일 앱 또는 Visual Studio Code에서 Cloud Shell에 액세스할 수 있습니다. 오른쪽 패널은 Cloud Shell 터미널이 연습 하는 동안 사용할 수 있습니다.
...
여기에서 사용 하 여는 `az` 데이터베이스 목록에 대 한 일부 정보를 표시 하는 유틸리티는 **물류** 해당 최대 크기 및 상태를 비롯 한 데이터베이스입니다.
...
1. 데이터베이스 이름만 필요하면 이 명령을 한 번 더 실행합니다. 이번에는 출력을 `jq`로 파이프하여 이름 필드만 출력합니다.
    ...
    ```
    **Logistics**는 데이터베이스입니다. SQL Server 등 **마스터** 로그인 계정 및 시스템 구성 설정과 같은 서버 메타 데이터를 포함 합니다.
...
1. 테이블에 있는 모든 행의 `DriverID` 열에서 이 `SELECT` T-SQL 문 목록을 실행합니다. 이것은 **읽기** 작업입니다.
