큐는 메시지 보낸 사람 응용 프로그램과 수신자 응용 프로그램에 해당 모양 라고 하는 데이터 패킷을 보관 합니다. 송신기는 큐를 만들고 메시지를 추가합니다. 수신기는 메시지를 검색하여 처리한 후 큐에서 해당 메시지를 삭제합니다. 다음 그림은 이 프로세스의 일반적인 흐름을 보여 줍니다.
