...
1. **Program.cs**를 선택하여 코드 편집기에서 엽니다.

1. `Microsoft.WindowsAzure.Storage` 네임스페이스를 참조하도록 파일 맨 위에 `using` 문을 추가합니다.
...
1. 여기서는 `await` 키워드를 사용할 것이므로 `Main` 메서드가 `async`이도록 서명을 변경하고 `Task`를 반환합니다.
...
::: zone-pivot="javascript" 저장된 연결 문자열을 사용하여 Azure 저장소 계정에 연결하는 코드를 추가하겠습니다. Azure 클라이언트 라이브러리는 자동으로 **AZURE_STORAGE_CONNECTION_STRING** 환경 변수를 사용하여 연결 문자열을 가져옵니다.
...
1. 다음으로, 바로 뒤에 **저장소** 개체를 사용하여 `BlobService` 개체를 만들고 글로벌 **blobService**에 저장합니다. 저장소 계정에 대한 액세스를 나타내는 경량 개체라는 사실을 기억하세요.

    ```javascript
    const blobService = storage.createBlobService();
    ```

1. 만들려는 컨테이너를 나타내는 상수를 추가합니다. 컨테이너 이름을 "photoblobs"로 지정하겠습니다.
...
`BlobService` 개체를 사용하여 Azure 저장소에서 BLOB API를 작업할 수 있습니다. 앞서 언급했듯이, 네트워크 호출을 만드는 모든 API는 비동기적으로 앱 응답성을 유지합니다. `createContainerIfNotExists` 메서드도 이러한 메서드 중 하나입니다. 우리는 _promises_를 사용하여 응답을 포함하는 콜백을 처리하겠습니다.

1. `util.promisify`를 사용하여 `createContainerIfNotExists`의 콜백 버전을 가져와서 promise-returning 메서드로 전환합니다.
    - 콜백 메서드는 개체에 있으므로 끝부분에 해당 컨텍스트에 연결하는 `bind` 호출을 추가해야 합니다.
...
1. "Hello, World!" 출력을 `main()`에서 제거합니다.

1. 새 `createContainerAsync` promise를 호출합니다.
    - **containerName** 상수에 전달합니다.
    - 호출에 `await` 키워드를 적용합니다.
    - 호출을 `try` / `catch` 구문에 래핑하고 아무 오류를 출력합니다.
...
1. `createContainerAsync` promise는 컨테이너 결과인 기본 `createContainerIfNotExists`의 첫 번째 값을 반환합니다. 여기에는 컨테이너 생성 여부에 대한 정보가 포함됩니다. 결과를 변수에 캡처하고 `result.created` 속성에 따라 컨테이너 생성 여부를 출력합니다.
...
1. Blob 패널에 **photoblobs** 컨테이너가 보일 것입니다. 항목의 오른쪽에 있는 "..." 메뉴를 통해 컨테이너를 삭제하고 앱에서 컨테이너를 다시 만들 수 있습니다.

> [!NOTE]
> 포털에서 컨테이너가 매우 빠르게 사라지지만, 실제로 삭제될 때까지 몇 분이 걸립니다. 컨테이너가 삭제되는 동안 컨테이너를 다시 만들려고 시도하면 Azure에서 오류 응답을 받게 됩니다.
