공용 인터넷을 통해 암호화된 터널을 사용하여 사용자 환경 내의 클라이언트 또는 사이트를 Azure에 연결할 수 있는지 확인하려고 합니다. 이 단원에서는 지점-사이트 간 VPN Gateway를 만든 다음, 클라이언트 컴퓨터에서 해당 게이트웨이에 연결합니다. 보안을 위해 네이티브 Azure 인증서 인증 연결을 사용합니다.
...
1. **내보낼 파일** 페이지의 **파일 이름**에서 **C:\cert\P2SRootCert.cer**을 입력한 후, 다음을 클릭합니다.
...
1. 이 명령의 출력에 반환된 URL을 복사하여 브라우저에 붙여넣습니다. 브라우저에서 .ZIP 파일 다운로드를 시작해야 합니다. 압축을 풀고 적절한 위치에 배치합니다.
