...
동일한 설정을 사용하여 VM 복사본을 만들려고 한다고 가정해 보겠습니다. VM 이미지를 만들어 Azure에 업로드하고 새 VM에 대한 기준으로 참조할 수 있습니다. 이 프로세스는 비효율적이며 시간 소모적입니다. Azure는 정확한 VM 복사본을 만드는 데 사용할 템플릿을 만드는 옵션을 제공합니다.

일반적으로 Azure 인프라에는 많은 리소스가 포함되며, 그 중 대부분은 몇 가지 방식으로 서로 관련되어 있습니다. 예를 들어 여기서 만든 VM에는 WordPress 사이트를 실행하기 위해 함께 만든 가상 머신 자체, 저장소, 네트워크 인터페이스, 웹 서버 및 데이터베이스가 모두 포함됩니다. **Azure Resource Manager**는 이러한 관련 리소스가 더 효율적으로 작동하게 합니다. 모든 리소스를 함께 배포, 업데이트 또는 삭제할 수 있는 명명된 **리소스 그룹**에 리소스를 구성합니다. WordPress 사이트를 만들었을 때 VM 만들기의 일부로 리소스 그룹을 식별했으며, Resource Manager에서 관련 리소스를 동일한 그룹에 배치했습니다.
...
나중에 사용할 리소스 템플릿을 저장하거나 이 템플릿에 따라 새 VM을 즉시 배포하는 옵션이 있습니다. 예를 들어 테스트 환경의 템플릿에서 VM을 만들 수 있으며 온-프레미스 머신을 교체하지 못할 수 있습니다. 리소스 그룹을 삭제하면 모든 리소스를 삭제하고 템플릿을 조정한 다음, 다시 시도할 수 있습니다. 기존의 배포된 리소스만 변경하려는 경우 해당 리소스를 만드는 데 사용된 템플릿을 변경하여 다시 배포할 수 있습니다. Resource Manager는 새 템플릿과 일치하도록 리소스를 변경합니다.

원하는 방식으로 작업한 후에는 해당 템플릿을 가져와서 준비 및 프로덕션과 같은 여러 버전의 인프라를 쉽게 다시 만들 수 있습니다. VM 이름, 네트워크 이름, 저장소 계정 이름 등과 같은 필드를 매개 변수화하고, 각 환경을 사용자 지정하기 위해 다른 매개 변수를 사용하여 템플릿을 반복적으로 로드할 수 있습니다.
...
관리 스크립트 만들기는 워크플로를 최적화하는 강력한 방법입니다. 일상적인 반복 작업을 자동화할 수 있고, 스크립트가 확인된 후에는 지속적으로 실행되어 오류가 감소할 수 있습니다. **Azure PowerShell**은 일회성 대화형 작업 및/또는 반복 작업 자동화에 적합합니다.
...
예를 들어 `New-AzureRmVM` cmdlet을 사용하여 새 Azure 가상 머신을 만들 수 있습니다.
...
**Azure CLI 도구로 가상 머신 관리** 모듈에서 VM을 만들고 관리하는 방법에 대해 자세히 알아보세요.

## <a name="programmatic-apis"></a>프로그래밍 방식(API)

일반적으로 실행할 간단한 스크립트가 있고 명령줄 도구를 계속 사용하려는 경우 Azure PowerShell과 Azure CLI는 모두 적절한 옵션입니다. VM 만들기 및 관리가 복잡한 논리를 사용하는 더 큰 응용 프로그램에 포함되는 더 복잡한 시나리오에 관한 또다른 접근 방식이 필요합니다.
...
Azure REST API는 VM을 만들고 관리하는 기능뿐만 아니라 리소스별로 분류된 작업도 개발자에게 제공합니다. 작업은 해당 HTTP 메서드(`GET`, `PUT`, `POST`, `DELETE` 및 `PATCH`) 및 해당 응답과 함께 URI로 노출됩니다.
...
Azure 클라이언트 SDK는 C#, Java, Node.js, PHP, Python, Ruby 및 Go와 같은 .NET 기반 언어를 포함한 다양한 언어 및 프레임워크에 사용할 수 있습니다.
...
알 수 있듯이, Azure는 리소스를 만들고 관리할 수 있는 다양한 도구를 제공하여 관리 작업을 _사용자에게 적합한_ 프로세스에 통합할 수 있습니다. 인프라 리소스가 원활하게 실행되고 있는지 확인하기 위해 다른 Azure 서비스 중 일부를 살펴보겠습니다.
