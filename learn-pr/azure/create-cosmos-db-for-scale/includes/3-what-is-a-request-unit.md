다음 데이터베이스에 대 한 전체에서 데이터를 생각해 보겠습니다. 적절 한 처리량이 중요 한 비즈니스 요구 사항에 대 한 트랜잭션 볼륨을 처리할 수 있습니다. 처리량 요구 사항이 항상 일관된 것은 아닙니다. 예를 들어 판매 또는 휴일 중에 크기를 조정해야 하는 쇼핑 웹 사이트를 빌드하고 있을 수 있습니다. 요청 단위 및 처리량 요구 사항을 예측 하는 방법에 관한 내용으로 시작 하겠습니다.
...
이제 요청 단위를 사용 하 여 Azure Cosmos DB에 대 한 예측 하는 방법 및 범위 처리량을 이해 하 고 새 Azure Cosmos DB 컬렉션을 만들 때 적합 한 선택 가능. 요청 단위를 언제 든 지 수정할 수 있지만 1000으로 설정 하는 계정을 만들 때 RU/s 보장 데이터베이스 나중에 확장할 준비가 되었습니다.