...
1. 이제 Program.cs 파일의 끝에 있는 **WriteToConsoleAndPromptToContinue** 함수에서 **CreateUserDocumentIfNotExists** 작업을 복사하고 붙여넣습니다.
...
1. 그런 다음, **BasicOperations** 메서드에 반환하고 해당 메서드의 끝에 다음을 추가합니다.
...
1. 통합 터미널에서 다시 다음 명령을 입력하여 프로그램을 실행합니다.

    ```csharp
    dotnet run
    ```

    응용 프로그램이 새 사용자 문서를 만들 때마다 터미널에 출력이 표시됩니다. 아무 키나 눌러 프로그램을 완료합니다.
...
1. 데이터베이스에서 문서를 읽으려면 다음 코드를 복사하여 Program.cs 파일의 WriteToConsoleAndPromptToContinue 메서드 뒤에 추가합니다.
...
1. 통합 터미널에서 다음 명령을 입력하여 프로그램을 실행합니다.
...
1. **ReplaceFamilyDocument** 메서드를 복사하여 Program.cs 파일의 ReadUserDocument 메서드 뒤에 붙여넣습니다.
...
1. 다음 코드를 복사하여 **BasicOperations** 메서드 끝에 붙여넣습니다.
