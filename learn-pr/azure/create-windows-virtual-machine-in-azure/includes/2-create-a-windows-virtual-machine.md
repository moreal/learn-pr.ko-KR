...
VM은 Azure에서 여러 가지 방법, 즉 Azure Portal, 스크립트(Azure CLI 또는 Azure PowerShell 사용) 또는 Azure Resource Manager 템플릿을 통해 정의하고 배포할 수 있습니다. 모든 경우에는 몇 가지 곧 다룰 정보를 제공 해야 합니다.

또한 Azure Marketplace는 특정 시나리오에 맞게 설치된 OS 및 인기 있는 소프트웨어 도구를 모두 포함하는 미리 구성된 이미지를 제공합니다.

![Azure Marketplace 가상 머신 목록을 보여주는 스크린샷.](../media/2-marketplace-vm-choices.png)
...
| **데이터 저장소 및 처리** 높은 디스크 처리량 및 IO는 빅 데이터, SQL 및 NoSQL 데이터베이스입니다. | Ls |
