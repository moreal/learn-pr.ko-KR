...
### <a name="leverage-consumption-pricing-models"></a>가격 책정 모델 활용 사용

PaaS 서비스를 이동 해도 true 소비 가격 책정 모델에 더욱 종 량 제 모델을 만들 수도 있습니다. Azure Functions와 같은 서비스를 사용 하는 기능이 _소비 계획_합니다. 소비 계획을 사용하는 경우 Azure Functions 호스트의 인스턴스는 들어오는 이벤트의 수에 따라 동적으로 추가되고 제거됩니다. 이 서버를 사용하지 않는 계획은 자동으로 규모를 조정하며, 함수를 실행하는 경우에만 계산 리소스에 대한 요금이 청구됩니다. 소비 계획에서 구성 가능한 시간 후 함수 실행 시간이 초과됩니다.

청구는 실행 횟수, 실행 시간 및 사용된 메모리를 기반으로 하며, 함수 앱 내의 모든 함수에 대해 집계됩니다.

소비 가격 책정 모델을 사용 하는 서비스 이동 비용 절감 아키텍처에는 새로운 방법으로 가져올 수 있습니다.
