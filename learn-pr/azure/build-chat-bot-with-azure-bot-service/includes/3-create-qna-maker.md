...
1. **이름** 상자에 “qa-factbot-kb”와 같은 이름을 입력합니다. 이 이름은 Azure 내에서 고유해야 하므로 블레이드 아래쪽에 있는 **앱 이름** 상자 *및* 그 옆에 녹색 확인 표시가 나타나야 합니다. 선택 **기존 항목 사용** 아래에서 **리소스 그룹**, 이전 연습에서 Azure web app 봇을 배포할 때 만든 "factbot rg" 리소스 그룹을 선택 하 고 있습니다. 가격 책정 계층으로 **F0** 및 **F**를 선택합니다. (둘 다 봇을 시험하기에 좋은 무료 계층입니다.) 두 위치 드롭다운에서 본인에게 가장 가까운 위치를 선택한 다음, 블레이드 아래쪽의 **만들기** 단추를 클릭합니다.

    ![에 설명 된 대로 구성 값을 사용 하 여 QnA Maker 만들기 블레이드를 보여 주는 Azure portal의 스크린샷](../media/3-new-qna-maker-service.png)
...
1. **Factbot.tsv**에서 가져온 질문과 답변이 기술 자료에 나타나는지 확인합니다. 그런 다음, **저장 후 학습**을 클릭하고 학습이 완료될 때까지 기다립니다.
    ...
    ![Factbot 자료 사이트 로드 기술 자료 데이터를 보여 주는 스크린샷.](../media/3-save-and-train.png)

1. **저장 후 학습** 단추 오른쪽에 있는 **테스트** 단추를 클릭합니다. 메시지 상자에 “Hi”를 입력하고 **Enter** 키를 누릅니다. 아래 그림과 같이 응답이 “QnA Factbot 시작”인지 확인합니다.

    ![만든된 채팅 봇과 테스트 상호 작용을 스크린샷.](../media/3-test-kb.png)
