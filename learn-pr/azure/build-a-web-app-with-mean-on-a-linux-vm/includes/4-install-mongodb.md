대부분의 응용 프로그램에는 데이터베이스가 필요합니다. MongoDB(MEAN 스택의 “M”)는 가장 인기 있는 NoSQL 데이터베이스 솔루션 중 하나이며 무료 오픈 소스입니다. NoSQL 데이터베이스는 SQL Server 또는 MySQL과 같은 관계형 데이터베이스처럼 미리 정의된 방식으로 데이터를 구조화할 필요가 없습니다.
