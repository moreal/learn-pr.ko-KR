...
첫째, 코드, 스크립트 및 응용 프로그램에 대 한 HTML 파일을 만들려면 하겠습니다. Cloud Shell 편집기에서이 작업을 수행 하 고 VM에 파일을 복사 합니다.

1. Cloud Shell에는 여전히 SSHed VM으로를 사용 하 여 `exit` Cloud Shell 파일 시스템으로 돌아갑니다.

    ```bash
    exit
    ```

1. 응용 프로그램에 대 한 파일과 폴더를 만들고 Cloud Shell 편집기에서 엽니다.
    ...
    이 프로젝트의 앱 및 해당 종속성을 포함 하도록 "Books" 라는 폴더를 만듭니다. 해당 폴더에서 모든 응용 프로그램 리소스와 스크립트가 포함되는 “app” 폴더를 만들었습니다. 마지막으로 적절한 HTTP 요청에 직접 제공될 클라이언트 쪽 파일이 모두 포함되는 "public" 폴더도 만들겠습니다.

## <a name="create-the-application"></a>응용 프로그램 만들기

1. Mongoose 데이터 모델을 만듭니다. 편집기에서 엽니다 `app/model.js` 다음 코드에 붙여 넣습니다.
    ...
    > [!IMPORTANT]
    > 코드 편집기에서 파일에 붙여 때마다 저장 해야 나중에 사용 하 여 `Ctrl+S`입니다.

    이 코드는 로컬 VM의 MongoDB 서버에 있는 "Books"라는 데이터베이스에 연결합니다. 그런 다음, `bookSchema` 변수로 정의된 스키마를 사용하여 "Books"라는 데이터베이스 문서를 만듭니다.

2. HTTP 요청을 처리 하는 Express 경로 만듭니다. 열기 `app/routes.js` 편집기에 다음 코드에 붙여 넣습니다.
...
3. 응용 프로그램을 호스트할 Express 서버를 만듭니다. 열기 `server.js` 편집기에 다음 코드에 붙여 넣습니다.
...
4. 클라이언트 쪽 JavaScript 응용 프로그램을 만듭니다. 열기 `public/script.js` 편집기에이 코드에 붙여 넣습니다.
...
5. 앱에 대 한 사용자 인터페이스를 만듭니다. 열기 `public/index.html` 편집기에이 코드에 붙여 넣습니다.
...
6. 파일 편집 완료 되었습니다. 모든 VM에 복사 하려면 다음 명령을 실행 하 고 저장 해야 합니다. 메시지가 표시되면 암호를 입력합니다.

    ```bash
    scp -r ~/Books <vm-admin-username>@<vm-public-ip>:~/Books
    ```

## <a name="install-node-packages"></a>노드 패키지 설치
...
1. **body-parser**를 설치하여 ExpressRoute 지정에 사용할 JSON 요청 데이터를 사전 처리합니다.
    ...
    > [!TIP]
    > 여러 npm 패키지를 설치할 때에 이와 같은 단일 명령으로 모두 포함할 수 있습니다.
    >
    > ```bash
