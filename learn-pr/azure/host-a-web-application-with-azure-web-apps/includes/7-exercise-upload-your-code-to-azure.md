이 연습에서는 Azure 웹앱에 ASP.NET Core 응용 프로그램을 업로드합니다.

## <a name="create-a-staging-deployment-slot"></a>준비 배포 슬롯 만들기

1. 이전에 만든 Web App 리소스를 엽니다. 해당 리소스 블레이드를 닫은 경우 **모든 리소스** 또는 **리소스 그룹**의 포함 리소스 그룹에서 앱을 검색하여 다시 찾을 수 있습니다.

1. 왼쪽 탐색 영역에서 **배포 슬롯** 메뉴 항목을 클릭합니다.

1. **배포 슬롯** 블레이드 내에서 블레이드의 위쪽 탐색 모음에 있는 **슬롯 추가** 단추를 클릭합니다.

1. Azure Portal에 아래와 같이 **슬롯 추가** 블레이드가 열립니다.

    1. 배포 슬롯에 이름을 지정합니다. 이 예제의 경우 `staging`입니다.

    2. **구성 원본** 선택 시에는 두 가지 옵션 중 하나를 사용할 수 있습니다.

        * 기존 배포 슬롯 또는 Azure에서 만든 Web App의 구성 요소를 복제할 수 있습니다.
...
1. 블레이드 아래쪽의 **확인** 단추를 클릭하여 새 배포 슬롯을 만듭니다.
...
1. Azure Portal에 새로 만든 배포 슬롯의 **개요** 페이지가 열립니다.
    ...
    배포 슬롯은 Azure 내의 전체 웹앱으로 처리됩니다. 그러나 이는 원래 웹앱의 자식인 특수한 유형이며 원래 웹앱으로 교환될 수 있습니다.
...
1. **배포 자격 증명** 블레이드의 위쪽에서 **저장** 단추를 클릭합니다.
...
다음으로는 코드 업로드를 시작할 수 있도록 Azure에서 로컬 Git 리포지토리를 만듭니다.

1. **스테이징** 배포 슬롯 웹앱의 왼쪽 탐색 영역에서 **배포 옵션** 메뉴 항목을 클릭합니다.

1. Azure Portal이 **배포 옵션** 블레이드로 이동합니다.

1. **원본 선택**을 클릭하여 필수 설정을 구성합니다.

1. Azure Portal에 구성하고 사용할 수 있는 사용 가능한 옵션이 표시됩니다. 이 경우 **로컬 Git 리포지토리** 옵션을 선택합니다.

1. **배포 옵션** 블레이드가 다시 열립니다. 블레이드 아래쪽의 **확인** 단추를 클릭하여 배포 원본을 설정합니다.
...
1. 이제 왼쪽 탐색 영역의 **배포 센터 (미리 보기)** 섹션으로 이동하여 새 배포 세부 정보를 확인합니다.
    ...
    여기서 확인해야 하는 중요한 정보는 **Git Clone URI**입니다. 이 URI는 로컬 웹 응용 프로그램 리포지토리의 **원격** 항목으로 사용할 로컬 Git 리포지토리 URL입니다.
...
1. 이전에 만든 웹앱의 콘텐츠 루트 폴더로 이동합니다. 다음을 입력하면 됩니다.
...
1. 위에서 생성된 스테이징 슬롯을 클릭합니다. 배포 슬롯은 웹앱으로 간주하므로 **모든 리소스** 아래에 웹앱 리소스로 표시됩니다.
...
1. 앞에서 생성된 원래 웹앱 블레이드로 이동합니다. 원래 웹앱은 **모든 리소스** 블레이드에서 확인할 수 있습니다.

1. 왼쪽 탐색 영역에서 **배포 슬롯** 메뉴 항목을 클릭합니다.

1. 블레이드 위쪽에서 **교환** 단추를 클릭합니다.
...
축하합니다. 웹앱을 Azure에 업로드하고 배포 슬롯을 교환했습니다.
